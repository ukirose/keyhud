...
なお、作ってから設計上の急所に気づきました。パネルが出た時点で、そこに載っていた項目は10秒間「調べた」判定になります。ところが ⌃ 系のショートカット（⌃A で行頭、⌃E で行末など）はエディタで ⌃ を押しっぱなしにして連打する使い方 が普通なので、連打のたびにパネルが出ていた頃は ⌃ 層が永久に加点されませんでした。補助のつもりのものが、自分の測定対象を壊していたわけです。直近の打鍵からの経過時間を条件に加えて解決しています。気づいた経緯は後の節に書きます。
