...
**品質面で分かっている穴があります。** テスト3件が、実行時の画面フォーカス状態で結果を変えます。パネル生成が実機のフォーカスを読むためで、入力欄にフォーカスがあるかどうかで通ったり落ちたりします。その3件については、緑であることが証拠になりません。テスト用の継ぎ目を入れて実機から切り離すのが正しい直し方です。
