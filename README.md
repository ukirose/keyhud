...
macOS の Secure Input（パスワード入力時などにキーイベントを他プロセスから隠す機能）でした。これは回避すべきものではありません。セキュリティ機能を迂回することになるからです。正しい対応は検出して表示することだと決めました。

報告先はパネルではありません。Secure Input が有効な間はパネル自体が出ないからです。イベントタップが修飾キーの押下を受け取る唯一の経路なので、タップが盲目になればホールドが始まりません。そこでメニューバーのアイコンに取り消し線を引き、メニューを開くと理由が読めるようにしました。AIが調査を尽くしても届かない範囲があり、画面に出ている情報を見るのはこちらの仕事だったという例です。
