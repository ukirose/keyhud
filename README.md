...
データは `~/Library/Application Support/KeyHUD/` に置かれます。外部への送信は一切ありません。

配色は初代 Macintosh に寄せてあります。System 7 の Platinum のベージュ、1bit 時代のグレーの作り方であるディザ（市松）、直角と1pxの枠、そして macOS に今も同梱されている Geneva。進捗の6マスは 1977年のロゴの6色で、覚えるほど左から埋まります。メニューバーから Ghostty の設定に追従させたり、他のテーマに変えたりもできます。
...
「このアプリの ⌘K って何だっけ」を、調べに行かずに済ませるものを作ろうと思いました。

もう一つ、裏の目的があります。覚えたショートカットと、毎回見ないと打てないショートカットを区別して記録することです。パネルを見てから打った操作は「調べた」、見ずに打った操作は「思い出した」として別に数え、後者だけが点数になります。

## なぜか

きっかけは自分の実測値でした。手元の Finder でキーボードから到達できるショートカットは 108 個ありますが、実際に使っているのはごく一部でした。全アプリで数えると 728 個中 24 個（3.3%）で、しかも上位5個が全使用の 80% を占めていました。

つまり困っているのは「ショートカットが無い」ことではなく、あるのに知らない、思い出せないことでした。

既存のチートシート系ツールは「呼び出すと一覧が出る」形式で、これはよくできています。ただ自分の場合、呼び出すこと自体を忘れます。そこで「修飾キーを押した時点で勝手に出る」形にしました。修飾キーを押しているのは、どうせショートカットを打とうとしている瞬間だからです。

学習記録を足したのは、一覧が出るだけだといつまでも一覧を見続けるからです。見ずに打てたときだけ加点することで、「もう見なくていいもの」が自分で分かるようにしました。

なお、作ってから設計上の急所に気づきました。パネルが出た時点で、そこに載っていた項目は10秒間「調べた」判定になります。ところが ⌃ 系のショートカット（⌃A で行頭、⌃E で行末など）はエディタで ⌃ を押しっぱなしにして連打する使い方 が普通で、パネルが 250ms で出てしまうと ⌃ 層は永久に加点されません。補助のつもりのものが、自分の測定対象を壊していました。気づいた経緯は後の節に書きます。
...
システム側のホットキー（⌃← で操作スペース移動など）も表示したいと考え、システム設定の画面をアクセシビリティ API で読ませようとしました。結果は要素数 0です。SwiftUI で作られた設定画面は、子要素を外部に公開していませんでした。

ここで「対応表を手で書き写す」案が出ましたが、それだと 押しても効かないキーを教えてしまう危険が残ります。方針を変えさせたところ、`SkyLight.framework` の `CGSGetSymbolicHotKeyValue` が見つかりました。ウインドウサーバに直接問い合わせるもので、設定ファイルに記録されない既定値も含めて実際の割り当てが返ります。226個のIDが応答しました。非公開APIなので `dlsym` で引き、見つからなければこの機能ごと黙って消える設計にします。
...
テスト3件が、実行するたびに通ったり落ちたりしました。原因は、パネルの生成がテキスト編集層を出すかどうかを決めるために、実機のウインドウサーバに「いまどこにフォーカスがあるか」を問い合わせていたことです。つまり、**suite を走らせた瞬間に入力欄が選択されていたかどうか**で結果が変わっていました。

その3件はどれも「どの行が描かれたか」を検査するものだったので、緑であること自体に意味がありませんでした。テキスト編集層を外から差し込める継ぎ目を作り、テストは空を渡すようにしています。

直したあとで、継ぎ目に実データを流し込むと**その3件だけが落ちる**ことも確認しました。そうしないと「フォーカスがたまたま外れていただけ」と区別が付きません。

## 次にやるなら何を変えるか

**閾値を、先に測ってから決めます。** 「修飾キーを押してから何ミリ秒でパネルを出すか」を 250ms と決めたのは、最初に何となく置いた数字でした。それが後から学習モデルを機能不全にしていました。この数字は今も暫定のままです。次にやるなら、閾値を置く前に自分の打鍵間隔の分布を測ります。体験を決める数字は、機能ではなく計測対象として扱うべきでした。
